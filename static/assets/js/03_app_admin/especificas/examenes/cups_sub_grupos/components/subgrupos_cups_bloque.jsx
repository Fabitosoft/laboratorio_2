...
import {
    SUBGRUPOS_CUPS as permisos_view,
...
class BloqueTab extends Component {
    ...
    onDelete(item, tipo) {
        ...
        const success_callback = () => {
            ...
            notificarAction('Cargando Grupos Cups');
        ...
        cargando();
        this.props.deleteCupsSubGrupo(item.id, success_callback, notificarErrorAjaxAction)
    }
    ...
    onSubmit(item, tipo) {
        ...
        const success_callback = () => {
            ...
            this.props.fetchCupsGrupos(cargarExamenes, notificarErrorAjaxAction)
        };
        cargando();
        if (item.id) {
            this.props.updateCupsSubGrupo(item.id, item, success_callback, notificarErrorAjaxAction);
        } else {
            this.props.createCupsSubGrupo(item, success_callback, notificarErrorAjaxAction);
        }
    }
    ...
    render() {
        ...
        return (
            <ListManager permisos={permisos} singular_name='sub grupo cups' plural_name='sub grupos cups'>
                {
                    (list_manager_state,
                     onSelectItem,
                     onCancel,
